%close all
%Parameters
Nx = 36; %number of X antennas
Ny = 36; %number of Y antennas
Nt = 36; %temporal Filter Order
Ns = 200; %number of time samples
ts = (1:Ns)';   
...
intpsi = 50*pi/180; %interference elevation
...
[W1,W2,W3] = meshgrid(w1,w2,w3);
f = patch(isosurface(W1,W2,W3, (abs(d3hw1w3).^2), 0.3));
isonormals(W1,W2,W3, abs(d3hw1w3), f);
...
w3=((-pi+2*pi/(Nt)):2*pi/(Nt):pi);
[W1,W2,W3] = meshgrid(w1,w2,w3);
f = patch(isosurface(W1,W2,W3, (abs(d3hw2w3).^2), 0.3));
isonormals(W1,W2,W3, abs(d3hw2w3), f);
...
title('Filtered signal as seen from an antenna (8x8x8)','fontsize',20,'FontName','arial');
