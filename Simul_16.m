...
Wbs = 16; % Signal bit Width
Fbs = 13; % signal fractional bit position
Wb = 16; %filter bit width filter
Fb = 13; %filter fractional bit position
...
% figure
% p = patch(isosurface(W1,W2,W3, abs(sigwintfr),0.5));
% isonormals(W1,W2,W3, abs(sigwintfr), p)
...
% title('3D frequency spectrum','fontsize',20,'FontName','arial')
% box on
% hold on
% w1=((-pi+2*pi/Nx):2*pi/Nx:pi);
% w2=((-pi+2*pi/Ny):2*pi/Ny:pi);
...
% [W1,W2,W3] = meshgrid(w1,w2,w3);
% f = patch(isosurface(W1,W2,W3, (abs(hw1w2w3).^2), 0.1),'FaceAlpha',.35);
% isonormals(W1,W2,W3, abs(hw1w2w3), f);
% set(f, 'FaceColor', 'blue', 'EdgeColor', 'none');
% xlabel('\omega_{1} Spatial');
% ylabel('\omega_{2} Spatial');
% zlabel('\omega_{3} Temporal');
...
% title('Signal seen at last antenna','fontsize',20,'FontName','arial');
% axis([0 150 -1.2 1.2]);
% xlabel('Time (Samples)','fontsize',16,'FontName','arial');
% ylabel('Magnitude','fontsize',16,'FontName','arial');
% figure
% mesh(abs(sigwintfr(:,:,round((3*Ns/4)))));
% title('Interference and Desired signal at slice pi/2');
% xlabel('Spatial Dimension N_1');
% ylabel('Spatial Dimension N_2');
...
figure
plot(time_output/(max(time_output)));
title('Filtered signal as seen from an antenna','fontsize',20,'FontName','arial');
xlabel('Time (Samples)','fontsize',16,'FontName','arial');
ylabel('Magnitude','fontsize',16,'FontName','arial');
axis([0 150 -1.2 1.2]);
